module.exports = function(sqlite3, DB_FILE) {
	...
	db.run('CREATE TABLE IF NOT EXISTS Server (_id INTEGER PRIMARY KEY AUTOINCREMENT, ownerId INTEGER, servername TEXT NOT NULL UNIQUE, serverpicture TEXT, FOREIGN KEY (ownerId) REFERENCES User(_id))');
	...
	db.run('CREATE TABLE IF NOT EXISTS ServerUser (_id INTEGER PRIMARY KEY AUTOINCREMENT, serverId INTEGER, userId INTEGER, roleId INTEGER, state TEXT, FOREIGN KEY (serverId) REFERENCES Server(_id), FOREIGN KEY (userId) REFERENCES User(_id), FOREIGN KEY (roleId) REFERENCES ServerRole(_id))');
